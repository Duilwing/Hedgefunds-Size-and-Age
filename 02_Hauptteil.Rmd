...
basedata_size <- filter(basedata_clean, `Fund Size USD`>0)

#auch hier anlegen der Daten, filtern nach Zeitinterval

size2005_2015 <- basedata_size %>% 
    mutate(`Obsolete Date` = coalesce(`Obsolete Date`,`Fund Size Date`))%>%
  filter(`Inception Date`< "2005-01-01")%>%
  filter(`Obsolete Date`> "2015-01-01")%>%
  filter(`Fund Size Date`> "2005-01-01")%>%
  filter(`Fund Size Date`> "2015-01-01")

#oder nur filtern nach ablesedatum der size
size2005_2015 <- basedata_size %>% 
    mutate(`Obsolete Date` = coalesce(`Obsolete Date`,`Fund Size Date`))%>%
  filter(`Fund Size Date`> "2015-01-01")
...
Im Anschluss wurden die weiteren Datensätze eingearbeitet und benannt. Zuerst der mri
...
#filtern nach Zeitinterval
mri05_15 <- mri%>%
  filter(date>= "2005-01-01")%>%
  filter(date< "2015-01-01")

#Löschen aller Funds mit NAs, also keinen Werten für mri

mri05_15[, 2:980][mri05_15[, 2:980] == "-NA"] <- NA
...
tidy_data <- pivot_longer(mri05_15, cols = 2:231, names_to ="fund", values_to = "mri")

#mri von character in numeric
tidy_data$mri <- as.numeric(tidy_data$mri)


#return,HWM und DD für mri

tidy_data <- tidy_data %>% 
  group_by(fund) %>% 
  mutate(lag_mri = lag(mri, n = 1), return = mri-lag_mri, hwm = cummax(mri), dd = (hwm - mri)/hwm)%>%
  ungroup() %>% 
  arrange(fund, date) %>%
  select(!lag_mri)
...
#mean return und volatility für gesamte Stichprobe

tidy_data %>% summarise(Mean = mean(return, na.rm = TRUE),
Volatility = sd(return, na.rm = TRUE))

#mean return und volatility für die funds

tidy_data %>% group_by(fund) %>% 
  summarise(Mean= 12*mean(return, na.rm = TRUE), Volatility=sqrt(12)*sd(return, na.rm = TRUE))


#VaR für einzelne funds noch dazu
tidy_data %>% 
  group_by(fund) %>% 
  summarise(Mean = 12*mean(return, na.rm = TRUE), Volatility = sqrt(12)*sd(return, na.rm = TRUE),
VaR = quantile(return, probs = 0.05,
na.rm = TRUE))

#Maximum DD noch dazu, abspeichern als summary
summary <- tidy_data %>% 
  group_by(fund) %>% 
...
#einlesen der daten
factordata <- read_csv("~/GitHub/Hedgefunds-Size-and-Age/DATA/factordata.csv")
...
ggplot(factordata05_15)+ geom_line(aes(x=date, y=mktrf), color="steelblue", size=1)+ theme_minimal()
```
