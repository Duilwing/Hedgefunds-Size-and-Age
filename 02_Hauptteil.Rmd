...
Age_Years < 4  ~ "young", Age_Years >= 4 & Age_Years < 8 ~ "medium", Age_Years >= 8 ~ "old"))%>%
...
size_final <- size_final%>%
 ...
 <= 50000000 ~ "small", `Fund Size USD` > 50000000  & `Fund Size USD` <= 250000000 ~ "medium", `Fund Size USD` > 250000000 ~ "big"))%>%
  arrange(secid, .by_group = FALSE)
...
```{r}
#filtern nach Zeitinterval 2005-2015
mri05_15 <- mri%>%
...
#pivot longer
tidy_data_s <- pivot_longer(mri10_20, cols = 2:980, names_to ="secid", values_to = "mri")

mrixd <- unique(mri10_20[2:980])<=60
...
summary_all_s <- tidy_data_s %>%
  group_by(secid)%>%
  summarise(Mean = mean(mri, na.rm = TRUE))
