...
#Luca: obsolete rausgenommen %>%filter(`Obsolete Date`> "2015-01-01")
...
#plot mit wes anderson farben
ggplot(age_final)+geom_bar(aes(x=Class,y = ..prop.., group = 1), stat = "count", fill=(values=wes_palette(n=3,"Darjeeling1")))+ scale_y_continuous(labels = scales::percent_format()) + theme_minimal()

```

Nach Filtern und Bestimmen der Klassen mit den Größen jung als unter zwei Jahre, medium als zwischen zwei und vier und alt als alle funds über vier ergibt sich die in Figure () dargestellte Verteilung. Auch wenn alte Funds mit einer Anzahl von 320 ca. 50% der Verteilung stellen, ist die Anzahl für junge (161) und mittlere (148) Funds ausreichend groß für eine weitere Analyse. Hier folgt die Arbeit der Einteilung von Jones(2007).

Um Größenvergleiche durchzuführen,werden aus dem Set alle Funds ohne einen Wert für size oder Net Asset Value gelöscht. Im Anschluss wird das Interval für alle Funds aktiv ab 2010 gebildet. Als Stichprobe ergeben sich ... aktive Funds.
...
#auch hier anlegen der Daten, filtern nach Zeitinterval ab 2010, dann Auswahl der Funds, die mindestens fünf Jahre in Interval aktiv
...
#plot mit wes anderson farben
ggplot(size_final_j)+geom_bar(aes(x=Class,y = ..prop.., group = 1), stat = "count", fill=(values=wes_palette(n=3,"Darjeeling1")))+ scale_y_continuous(labels = scales::percent_format()) + theme_minimal()


```

Die Darstellung der Verteilung mit Klassenbildung nach Jones zeigt eine klare Verzerrung hin zu kleinen Funds. So bilden die kleinen Funds ca. 80% der Verteilung, während große Funds mit ...% unterrepräsentiert sind. Somit ist keine Analyse für Funds ausserhalb der Kategorie klein gegeben.
...
size_final <- size_final%>%
  ...
  arrange(secid, .by_group = FALSE)%>%
  select(name, secid,`Fund Size USD`, `Net Assets - Share Class USD`, Quartile_Class)

#plot mit size

ggplot(size_final)+geom_col(aes(x=Quartile_Class, y= `Fund Size USD`)) + theme_minimal()
...
```{r}
#Für Size: filtern nach Zeitinterval 2010-2020
mri10_20 <- mri%>%
...
#Luca: hier bei fünf funds für MDD ein problem, die werden noch rausgelöscht
...
#erstellen eines großen Datensets mit Klassen nach den 5 Quartilen 
size_final_summary <- merge(size_final, summary_s, by = "secid")%>%
  arrange(secid, .by_group = FALSE)%>%
  select(secid, Quartile_Class, Mean, Volatility, VaR, MDD )


ggplot(size_final_summary)+geom_bar(aes(x=Quartile_Class), fill="steelblue")+theme_minimal()
...
#erstellen eines großen Datensets mit Klassen von klein,mittel, groß
sizej_final_summary <- merge(size_final_j, summary_s, by = "secid")%>%
  arrange(secid, .by_group = FALSE)%>%
  select(secid, Class, Mean, Volatility, VaR, MDD )

#Erstellen der einzelnen Zusammenfassungen für die Klassen
summary_small_j <- sizej_final_summary%>%
  filter(Class=="small")%>%
  summarise(Mean = mean(Mean), Volatility = mean(Volatility), VaR = mean(VaR),
MDD = mean(MDD, na.rm=TRUE))%>%
  mutate(Class="small")%>%
...
summary_medium_j <- sizej_final_summary%>%
  filter(Class=="medium")%>%
  summarise(Mean = mean(Mean), Volatility = mean(Volatility), VaR = mean(VaR),
MDD = mean(MDD, na.rm=TRUE))%>%
  mutate(Class="medium")%>%
...
summary_big_j <- sizej_final_summary%>%
  filter(Class=="big")%>%
  summarise(Mean = mean(Mean), Volatility = mean(Volatility), VaR = mean(VaR),
MDD = mean(MDD, na.rm=TRUE))%>%
  mutate(Class="big")%>%
