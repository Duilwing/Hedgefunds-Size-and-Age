...
basedata <- select(basedata, name, secid, `Inception Date`,`Obsolete Date`, `Fund Size Date`, `Fund Size USD`)
...
Age_Years < 4  ~ "young", Age_Years >= 4 & Age_Years < 10 ~ "medium", Age_Years >= 10 ~ "old"))%>%
...
tidy_data <- pivot_longer(mri05_15, cols = 2:231, names_to ="secid", values_to = "mri")

#mri von character in numeric
tidy_data$mri <- as.numeric(tidy_data$mri)


#return,HWM und DD für mri für die einzelnen funds
...
summary_young <- age_final_summary%>%
  filter(Class=="young")%>%
  summarise(Mean_young = mean(Mean), Volatility_young = mean(Volatility), VaR_young = mean(VaR),
MDD_young = mean(MDD))

summary_medium <- age_final_summary%>%
  filter(Class=="medium")%>%
  summarise(Mean_medium = mean(Mean), Volatility_medium = mean(Volatility), VaR_medium = mean(VaR),
MDD_medium = mean(MDD))


summary_old <- age_final_summary%>%
  filter(Class=="old")%>%
  summarise(Mean_old = mean(Mean), Volatility_old = mean(Volatility), VaR_old = mean(VaR),
MDD_old = mean(MDD))
