...
```{r, include=FALSE, warning=FALSE,fig.cap = "Histogramm der Rendite für 'Alter'"}
...
```{r, include=FALSE, warning=FALSE,fig.cap = "Histogramm der Rendite für 'Größe'"}
...
```{r, echo=FALSE, warning=FALSE,fig.cap = "Histogramm der Rendite für 'Größe'"}

p1 <- ggplot(data = tidy_return_age, aes(x=ret)) +
  geom_histogram(aes(y =..density..),fill="#440154FF",alpha=0.95,binwidth = 0.001)+theme_minimal()+
scale_x_continuous(limits =c(-0.4,0.4))+ 
stat_function(fun = dnorm,args = list(mean = mean(tidy_return_age$ret, na.rm = TRUE), sd = sd(tidy_return_age$ret, na.rm = TRUE)),aes(colour = "normal distribution"), size=0.7)+
geom_density(aes(color="distribution"),size=0.7)+ scale_colour_manual("", values = c("#287D8EFF","#FDE725FF"))

p2 <- ggplot(data = tidy_return_size, aes(x=ret)) +
  geom_histogram(aes(y =..density..),fill="#440154FF",alpha=0.95,binwidth = 0.001)+theme_minimal()+
scale_x_continuous(limits =c(-0.4,0.4))+ 
...
ggarrange(p1, p2, ncol = 2, label.y = "density", common.legend = TRUE)

```

Abbildung 1 zeigt die Histogramme der Rendite für beide Datensätze sowie in gelb die Normalverteilung. Beide Abbildungen bestätigen das Ergebnis der Tests da zu hohe Kurtosis vor, die Renditen also leptokurtisch verteilt sind.
...
Im Anschluss werden die Fonds Größe und Alter entsprechend in Gruppen nach @Jones.2007 eingeteilt: 'jung' entspricht einem Alter von bis zu zwei Jahren, 'medium' zwischen 2 und 4 Jahren sowie 'alt' ab einem Alter von mindestens 5 Jahren. Hinsichtlich der Größe: klein (< 100 Mio. USD), medium (100 - 500 Mio. USD) und groß (> 500 Mio. USD). Die Verteilung beider Datensätze kann Abbildung 2 entnommen werden. 

```{r, include=FALSE, warning=FALSE,fig.cap = "Klassenverteilung innerhalb der Age-Stichprobe"}
...
```{r,include=FALSE, warning= FALSE, fig.cap = "Klassenverteilung innerhalb der Size-Stichprobe"}
...
Auch wenn alte Fonds mit einer Anzahl von 194 ca. 50% der Verteilung stellen, ist die Anzahl für junge (79) und mittlere (132) Fonds ausreichend groß für eine weitere Analyse. Hinsichtlich des Datensatzes für 'Alter' zeigt die Verteilung eine klare Verzerrung hin zu kleinen Fonds (80%), während große Fonds (4,5%) unterrepräsentiert sind. Daher erfolgt im weiteren Verlauf der Arbeit die Einteilung nach Größe in Quintile, ähnlich dem Ansatz von @Howell.2001.
...
```{r, include=FALSE, warning=FALSE,fig.cap = "Zusammenhang von Volatilität und Mean innerhalb der Age-Stichprobe"}
...
Bis auf eine Ausnahme (Q5) kleine negative Werte könnten eine hohe Volatilität und damit idiosynchratisches Risiko in der zweiten Stichprobe widerspiegeln.
->Pi: was ist mit diesem Satz gemeint?
