---
title: "Jung gegen Alt: Einfluss von Alter und Größe auf die Performance von Hedgefunds"
author1: "Luca "
adress_street_and_no1: "Königsworther Platz 1"
postcode1: "30167"
place1: "Hannover"
matriculation_number1: "1234567"
semester1: "1"
course_of_study1: "Wirtschaftswissenschaft"
# if you write in a team, put the data of the second author here:
author2: "Melissa Musterfrau"
adress_street_and_no2: "Königsworther Platz 2"
postcode2: "30167"
place2: "Hannover"
matriculation_number2: "7654321"
semester2: "2"
course_of_study2: "Wirtschaftswissenschaft"
date: "31.03.2022"
university: "Leibniz University Hannover"
faculty: "School of Economics and Management" 
institute: "Institute of Banking and Finance"
advisor: "Prof. Dr. Maik Dierkes"
altadvisor: "Dr. Sebastian Schrön"            #Pi: Ist ja mittlerweile Dr. der Gute
module: "Hedge Funds: Trading Strategies and Performance Evaluation"
# Belegnummer:
module_number: "374040"
lang: "de"
output: 
  pdf_document:
    template: ibf_template.tex
    extra_dependencies:
    - dcolumn
    toc: true
    toc_depth: 3
    highlight: tango
    keep_tex: false
    number_sections: true
geometry: "a4paper, left=2cm,right=5cm,top=3cm,bottom=3cm"
bibliography: BIB/example_bibliography.bib
csl: CSL/apa.csl
fontsize: 11pt
linestretch: 1.5
lof: true
lot: true
editor_options: 
  markdown: 
    wrap: sentence
---

```{r setup, include=FALSE, warning= FALSE}
knitr::opts_chunk$set(echo = TRUE, message = FALSE)



sharkdata <- "https://raw.githubusercontent.com/allisonhorst/stats-illustrations/master/other-stats-artwork/shark_raw.csv"

#Pi: das kann raus - hatte er nur für den Graphen in Hai-Form drin


library(knitr)
library(tidyverse)
library(tinytex) #Pi: sonst funktioniert das knitten nicht

```

# Einleitung

```{r child='01_Einleitung.Rmd'}
```

# Hauptteil

```{r child='02_Hauptteil.Rmd'}
```

# Fazit

```{r child='03_Fazit.Rmd'}
```

# Tabellenverzeichnis

```{r child='04_Tabellenverzeichnis.Rmd'}
```

\newpage

# Literaturverzeichnis

```{r child='05_Literaturverzeichnis.Rmd'}
```